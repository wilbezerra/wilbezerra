...
if "%CHOICE%"=="1" (
    set SCRIPT=WSBFlashClean v3.1(SAFE) - PUBLIC PROTECTED.ps1
    set OUTPUT=WSBFlashClean_SAFE_Public.exe
    goto build
)

if "%CHOICE%"=="2" (
    set SCRIPT=WSBFlashClean v3.1 - AGGRESSIVE PUBLIC PROTECTED.ps1
    set OUTPUT=WSBFlashClean_Aggressive_Public.exe
    goto build
)
...
:build
set ICON=wsb_flash_clean_v3.0.ico
