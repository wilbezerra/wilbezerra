# ==============================================================================
#  WSB AUTO CLEAN (SAFE PUBLIC PROTECTED) - GHOST EDITION  v3.3.2
...
# ==============================================================================
function Get-WSBAutoCleanSelfHash {
    try {
...
$Script:WSBCurrentHash  = Get-WSBAutoCleanSelfHash
...
if ($Script:WSBExpectedHash -ne "PLACEHOLDER_HASH") {
    if ([string]::IsNullOrWhiteSpace($Script:WSBCurrentHash) -or ($Script:WSBCurrentHash -ne $Script:WSBExpectedHash)) {
        try {
            $violDir = Join-Path $env:USERPROFILE "Documents\WSB Auto Clean GE"
...
$Script:MarcadoresAutoExcluidos = @(
    ...
    '\userdata\','\tdata\','\OneAuth\','\TokenBroker\','\Microsoft\Protect\',
    '\Microsoft\Credentials\','\Windows\WebCache\','\Packages\'
)
...
function Testar-Pasta-Cache-AutoSegura {
    ...
    # Caches de risco baixo: pode liberar direto.
    if ($Script:NomesCacheAutoDiretos -contains $nomeNormal) { return $true }

    # Nomes mais ambiguos como temp/tmp/logs/caches so entram com heuristica extra.
...
# [PATCH SAFE - ICONES]
# Versao segura para o AutoClean: mantem a limpeza de caches visuais, mas evita
# reiniciar o Explorer e nao remove Shell Bags/BagMRU, preservando tamanho,
# organizacao automatica e layout da area de trabalho.
function Executar-Limpeza-Icones {
    ...
    try { ie4uinit.exe -ClearIconCache 2>&1 | Out-Null } catch {}
    ...
    # Preserva explicitamente as chaves de layout do shell.
    $sb = -1
...
public static class WSBIconSafeRefresh {
...
"@ -ErrorAction SilentlyContinue
    ...
    try { [WSBIconSafeRefresh]::SHChangeNotify(0x08000000, 0x0000, [IntPtr]::Zero, [IntPtr]::Zero) } catch {}
    try { [WSBIconSafeRefresh]::SHChangeNotify(0x2000000, 0x0000, [IntPtr]::Zero, [IntPtr]::Zero) } catch {}
...
if ($AutoRun) {
    ...
    $logIco = Gravar-Log -Operacao "LIMPEZA ÍCONES" -Linhas @(
        ...
        "  Explorer          : Preservado (modo seguro)",
        # [PATCH SEGURO - EXPLORER]
        # Log explicito para facilitar auditoria sem alterar o fluxo funcional da rotina.
        "  Janela Inicial    : Nao aplicada (modo seguro)",
        "  Resultado         : OK"
    )
    ...
    Enviar-Toast `
        ...
        -Corpo  "Iconcache + Thumbcache + JumpLists Limpos  |  Explorer Preservado  |  Log Salvo" `
        -Modo   "Icones" `
        -Audio  "Notification.Default"
